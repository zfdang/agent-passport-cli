# Combined Token And Encrypted Profile Migration Notes

> **状态**：历史实现说明 / 已基本落地
>
> 这份文档原本是 Combined Token、加密本地身份存储、以及 CAIP-2
...
- `kitepass access-key create` 只展示一次 Combined Token
- CLI 不会把 Combined Token 直接写入本地文件
- 运行时通过 `KITE_AGENT_TOKEN` 读取它
- Token 中的 `access_key_id` 用于匹配本地 profile
...
- owner access token 以加密 envelope 形式存入 `config.toml`
...
- `README.md`
- `docs/cli-manual.md`
- `docs/owner-token-and-agent-access-key-flow.md`
