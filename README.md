...
PASSPORT_ID="$(printf '%s' "$BOUND_KEY_JSON" | jq -r '.agent_passport_id')"
...
The full Passport Token is already available in the current shell as `KITE_AGENT_PASSPORT_TOKEN`. That is the value an agent should use.
